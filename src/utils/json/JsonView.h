#ifndef UTILS_JSONVIEW_H_
#define UTILS_JSONVIEW_H_
...
class JsonView {
    ...
    private:
        ...
        const char *data               = nullptr;  ///< The data to parse.
    ...
    public:
        ...
        class JsonNode {
            ...
            public:
                ...
                class JsonObject : private std::vector<std::pair<std::string_view, JsonNode>> {

                    private:
                        using Parent = std::vector<std::pair<std::string_view, JsonNode>>;

                        friend class JsonView;
                        friend struct JsonParser;

                        void insert(std::string_view key, JsonNode &&other) { Parent::push_back(std::make_pair(key, std::move(other))); }

                    public:
                        JsonObject() = default;

                        template<typename T>auto find(const T &key) const {
                            return std::find_if(Parent::cbegin(), Parent::cend(), [&key](const auto &kv) { return kv.first == key; });
                        }

                        template<typename T>const JsonNode& at(const T &key) const {
                            if (const auto it = find(key); it != Parent::cend())
                                return it->second;
                            throw std::runtime_error{ "Not found." };
                        }

                        template<typename T>JsonNode& at(const T &key) {
                            auto it = std::find_if(Parent::begin(), Parent::end(), [&key](const auto &kv) { return kv.first == key; });
                            if (it != Parent::end())
                                return it->second;
                            throw std::runtime_error{ "Not found." };
                        }

                        template<typename T>void erase(const T &key) {
                            if (const auto it = find(key); it != Parent::cend())
                                Parent::erase(it);
                        }
            ...
            public:
                ...
                template<typename T>
                const JsonNode& value_or_null(const T &key) const noexcept {
                ...
                template<typename T>const JsonNode& at(const T &key) const { return const_cast<const JsonObject*>(std::get<JsonObject*>(data))->at(key); }         
                template<typename T>JsonNode& at(const T &key) { return std::get<JsonObject*>(data)->at(key); }
    ...
    public:
        ...
        JsonView(const char *data, ParseMethod method = STANDARD) : data{ data }, r{ method == NOTHROW ? parse(STANDARD) : parse_throw_on_error(method) } { }

        JsonView(const JsonView&) = delete;
        JsonView& operator=(const JsonView&) = delete;

        JsonView(JsonView&&) = default;
        JsonView& operator=(JsonView&&) = default;
    ...
    private:
        ...
        struct JsonParser {
            ...
            public:
                ...
                JsonParser(const char *str, std::size_t N, ParseMethod strategy) : str{ str }, N{ N }, strategy{ strategy } {}
                JsonParser(const char *str, ParseMethod strategy) : str{ str }, N{ std::strlen(str) }, strategy{ strategy } {}
            ...
            private:
                ...
                static std::string_view slice(const char *start, std::size_t len) {
                ...
                template <typename T>T fail(std::string &&msg, T err_ret) {
                    if (!failed)
                        err = std::move(msg);
                    failed = true;
                ...
                JsonNode parse_json(int depth) {
                    ...
                    if (ch == '{') {
                        ...
                        while (1) {
                            ...
                            data.insert(std::move(key), std::move(val));
