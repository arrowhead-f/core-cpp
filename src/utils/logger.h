...
//            using me = log_stream_op<S, str...>::template stream<Args...>;
            ...
            template<typename V>
            auto next(V &&val) && {
                return stream<Args..., V>{ std::tuple_cat( std::move(args), std::tie(val) ) };
            }
...
//    }
    ...
    #ifndef USE_NILLOG
    template<typename X, typename T> auto operator<<(X &&lstr, const T &val) {
        return std::move(lstr).next(val);
    }
    #endif
