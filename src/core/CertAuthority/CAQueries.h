...
template<typename DB>class CAQueries {
    ...
    public:
        ...
        long getKeyCount() const {
            const std::string q = "SELECT COUNT(*) FROM ca_trusted_key";
            (debug{ } << fmt("CAQuery: {}") << q).log(SOURCE_LOCATION);
        ...
        auto getKeys(const std::string &page, const std::string &item_per_page, const std::string &sort_by, const std::string sort_dir) const {

            std::string q = "SELECT id, created_at, description FROM ca_trusted_key ORDER BY " + sort_by + " " + sort_dir;
            if (!page.empty()) {
                const auto p = std::stoul(page);
                const auto i = std::stoul(item_per_page);
                const auto o = p * i; // calculate the offset
